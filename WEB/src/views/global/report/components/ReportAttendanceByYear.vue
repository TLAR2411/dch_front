...
const selectedYear = computed(() =>
  yearOptions.value.find((y) => Number(y.id) === Number(yearId.value)) ?? null,
);
...
function getYearDateRange(terms, year) {
  if (terms.length) {
    const startDates = terms
      .map((term) => normalizeDate(term.start_date))
      .filter(Boolean)
      .sort();
    const endDates = terms
      .map((term) => normalizeDate(term.end_date))
      .filter(Boolean)
      .sort();
  ...
  return {
    startDate: normalizeDate(year?.start_date),
    endDate: normalizeDate(year?.end_date),
  };
}
...
const canSearch = computed(() => Boolean(props.class_id && yearId.value));
...
async function fetchTermsForYear(yearId) {
  ...
  try {
    ...
    termOptions.value = (res.data?.data?.data ?? [])
      .filter((term) => Number(term.year_id) === Number(yearId))
...
async function fetchReportData() {
  await fetchTermsForYear(yearId.value);
...
onMounted(async () => {
  ...
  yearOptions.value = app()?.years ?? [];

  if (!yearStore.year_id && yearOptions.value.length) {
    yearStore.setYearId(yearOptions.value[yearOptions.value.length - 1].id);
  }
